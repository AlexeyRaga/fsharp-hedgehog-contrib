...
By separating generation from execution, Hedgehog can:

1. **Build structurally valid sequences** using only the model state, without touching your real system
2. **Reject invalid shrink attempts immediately** by checking preconditions symbolically
3. **Only execute sequences that make sense**, minimizing load on your system
...
### Example: The Efficiency Gain

Consider testing a stack with Push and Pop commands. During shrinking, Hedgehog might try:

```text
Original (fails): Push(1), Push(2), Push(3), Pop, Pop, Pop, BuggyOperation
Shrink attempt 1: Pop, Pop, Pop, BuggyOperation  ← REJECTED at generation (can't pop empty stack)
Shrink attempt 2: Push(1), Pop, Pop, BuggyOperation  ← REJECTED at generation (second pop invalid)
...
- Parallel testing (where invalid interleavings can be rejected without execution)
...
3. Update       → Update model state symbolically (same method as generation, now env has real values)
